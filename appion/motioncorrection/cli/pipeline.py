...
def pipeline(tasklist: list, args : dict, jobmetadata: dict, client : Client, retries : int = 3):
    ...
    for imageid in tasklist:
        ...
        task_f=client.submit(lambda pretask_data : motioncor(**pretask_data[0]), pretask_f, pure=False, retries=retries, resources={'GPU': 1})
        futures.append(task_f)

        posttask_f=client.submit(lambda pretask_data, task_data : postTask(imageid, pretask_data[0], pretask_data[1], jobmetadata, args, task_data[0]), pretask_f, task_f, pure=False, retries=retries)
